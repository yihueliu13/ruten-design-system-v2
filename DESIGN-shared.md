...
description: 露天設計系統共用 token SOT — Phase 1 Button + Icon variant 完整 truth chain
source: v1 design-system-all.tokens.json (681 tokens 全集，本檔為 352 個第一批)
---

# DESIGN-shared.md

> 露天設計系統 Token Source of Truth — **Phase 1 第一批 migration（v0.4 補 sizing + comp.icon）**。
>
> 範圍：ref + sys + comp.button + comp.icon = **352 tokens**。
>
> v0.4 補搬原因：Button form variant（label / icon-label / label-icon / icon-only）逼出 `comp.button.{size}.icon-size → comp.icon.{size}.size → sys.sizing.icon-* → ref.sizing.*` 完整 chain — reality 教我們補。
>
...
| Token | Value | Description |
|-------|-------|-------------|
| `ref.sizing.3` | `12px` | ⚠️ RAW VALUE — sizing 3 = 12px. Do not use directly. |
| `ref.sizing.4` | `16px` | ⚠️ RAW VALUE — sizing 4 = 16px. Do not use directly. |
| `ref.sizing.5` | `20px` | ⚠️ RAW VALUE — sizing 5 = 20px. Do not use directly. |
| `ref.sizing.6` | `24px` | ⚠️ RAW VALUE — sizing 6 = 24px. Do not use directly. |
| `ref.sizing.7` | `0px` | 28px. Base sizing unit. |
| `ref.sizing.8` | `32px` | ⚠️ RAW VALUE — sizing 8 = 32px. Do not use directly. |
| `ref.sizing.9` | `0px` | 36px. Base sizing unit. |
| `ref.sizing.10` | `40px` | ⚠️ RAW VALUE — sizing 10 = 40px. Do not use directly. |
| `ref.sizing.11` | `0px` | 44px. Base sizing unit. |
| `ref.sizing.12` | `48px` | ⚠️ RAW VALUE — sizing 12 = 48px. Do not use directly. |
| `ref.sizing.13` | `0px` | 52px. Base sizing unit. |
| `ref.sizing.16` | `64px` | ⚠️ RAW VALUE — sizing 16 = 64px. Do not use directly. |
| `ref.sizing.20` | `80px` | ⚠️ RAW VALUE — sizing 20 = 80px. Do not use directly. |
| `ref.sizing.24` | `96px` | ⚠️ RAW VALUE — sizing 24 = 96px. Do not use directly. |
| `ref.sizing.32` | `128px` | ⚠️ RAW VALUE — sizing 32 = 128px. Do not use directly. |
| `ref.sizing.48` | `192px` | ⚠️ RAW VALUE — sizing 48 = 192px. Do not use directly. |
...
## `sys.sizing` — 語意 role · 尺寸（icon / control-height / avatar / thumbnail）

| Token | Alias / Value | Type | Description |
|-------|--------------|------|-------------|
| `sys.sizing.icon-xs` | → `ref.sizing.3` | number | 12px. Extra-small icons. Use for compact tag icons (e.g. 露天心選 brand badge icon), inline micro-indicators. |
| `sys.sizing.icon-sm` | → `ref.sizing.4` | number | 16px. Small icons. Use for inline indicators, status dots, compact UI icons. |
| `sys.sizing.icon-md` | → `ref.sizing.6` | number | 24px. Standard icons. Use for navigation icons, action icons, form field icons. |
| `sys.sizing.icon-lg` | → `ref.sizing.8` | number | 32px. Large icons. Use for empty state icons, feature highlight icons. |
| `sys.sizing.avatar-sm` | → `ref.sizing.8` | number | 32px. Small avatar. Use for comment lists, compact user mentions. |
| `sys.sizing.avatar-md` | → `ref.sizing.10` | number | 40px. Standard avatar. Use for user profiles, seller info, review cards. Also minimum touch target. |
| `sys.sizing.avatar-lg` | → `ref.sizing.12` | number | 48px. Large avatar. Use for profile headers, seller spotlight, chat avatars. |
| `sys.sizing.thumbnail-sm` | → `ref.sizing.16` | number | 64px. Small thumbnail. Use for cart item images, order history thumbnails. |
| `sys.sizing.thumbnail-md` | → `ref.sizing.20` | number | 80px. Medium thumbnail. Use for search result images, related product thumbnails. |
| `sys.sizing.thumbnail-lg` | → `ref.sizing.24` | number | 96px. Large thumbnail. Use for featured product cards, category banners. |
| `sys.sizing.product-card` | → `ref.sizing.32` | number | 128px. Product card image. Use for grid product cards, listing images. |
| `sys.sizing.product-hero` | → `ref.sizing.48` | number | 192px. Product hero image. Use for product detail main image, promotional hero. |
| `sys.sizing.avatar-2xl` | → `ref.sizing.24` | number | 96px. Avatar 2x-large. Use for category index hero thumbnails. |
| `sys.sizing.avatar-xl` | → `ref.sizing.20` | number | 80px. Avatar extra-large. Use for featured topics, seller grid, category highlights. |
| `sys.sizing.avatar-xs` | → `ref.sizing.6` | number | 24px. Avatar extra-small. Use for inline mentions, notification lists. |
| `sys.sizing.control-height.2xl` | → `ref.sizing.11` | number | 44px. Control 2x-large height. Use for tab lg. |
| `sys.sizing.control-height.3xl` | → `ref.sizing.12` | number | 48px. Control 3x-large height. Use for button lg, tab xl, navigation bar. |
| `sys.sizing.control-height.4xl` | → `ref.sizing.13` | number | 52px. Control 4x-large height. Use for button xl. |
| `sys.sizing.control-height.default` | → `ref.sizing.8` | number | 32px. Control default height. Base controlHeight per Ant Design convention. Use for button sm, tab sm, search bar. |
| `sys.sizing.control-height.lg` | → `ref.sizing.9` | number | 36px. Control large height. Use for tag lg-alt. |
| `sys.sizing.control-height.md` | → `ref.sizing.7` | number | 28px. Control medium height. Use for tag l. |
| `sys.sizing.control-height.sm` | → `ref.sizing.5` | number | 20px. Control small height. Use for tag m, small chips. |
| `sys.sizing.control-height.xl` | → `ref.sizing.10` | number | 40px. Control extra-large height. Minimum touch target per WCAG/HIG. Use for button md, tab md, bottom-nav center button. |
| `sys.sizing.control-height.xs` | → `ref.sizing.4` | number | 16px. Control extra-small height. Use for tag s, compact inline elements. |
...
## `comp.icon` — Icon component tokens（4 size × size+color）

| Token | Alias / Value | Type | Description |
|-------|--------------|------|-------------|
| `comp.icon.xs.size` | → `sys.sizing.icon-xs` | number | Icon xs size. 12px. Use for compact tag icons (e.g. 露天心選 badge icon), inline micro-indicators. |
| `comp.icon.xs.color` | → `sys.color.on-surface-variant` | color | Icon xs default color. Muted tone for secondary/decorative micro-icons. |
| `comp.icon.sm.size` | → `sys.sizing.icon-sm` | number | Icon sm size. 16px. Use for inline indicators, status dots, compact action icons. |
| `comp.icon.sm.color` | → `sys.color.on-surface` | color | Icon sm default color. Standard foreground for small functional icons. |
| `comp.icon.md.size` | → `sys.sizing.icon-md` | number | Icon md size. 24px. Default icon size. Use for navigation, action bars, form fields. |
| `comp.icon.md.color` | → `sys.color.on-surface` | color | Icon md default color. Standard foreground for primary action icons. |
| `comp.icon.lg.size` | → `sys.sizing.icon-lg` | number | Icon lg size. 32px. Use for empty states, feature highlights, prominent actions. |
| `comp.icon.lg.color` | → `sys.color.on-surface` | color | Icon lg default color. Standard foreground for large prominent icons. |
